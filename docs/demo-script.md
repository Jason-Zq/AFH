...
（讲解点：首页居中一句话说清架构——Researcher → Analyst → Writer ⇄ Reviewer 图编排，本地文档库 30 篇 + 博查联网双通道。）
...
- "左侧过程流第一张卡片是 Researcher（顶部阶段条同步点亮），它是唯一的 **HarnessAgent**——规划、工具调用循环都由 Harness 托管。"
...
（报告出现在右栏）"通过！右栏就是定稿的 Markdown 报告，带目录导航，可以下载。"

## 第二幕：审校打回（2 分钟，可选，需要运气）

审校是否打回取决于模型判断，不保证现场出现。**两种方案**：

- **方案 A（现场碰运气）**：换个更宽泛的问题，如 `聊聊多智能体编排`，Reviewer 更可能因"来源不足"打回。若出现 Writer 第二次出现，立刻指着讲："看，阶段条上 Writer 亮起了「第 2 轮」徽标，Writer 被执行了第二轮——这就是图编排的回环，顺序流水线做不到。"
...
**操作**：回到首页（侧栏"新研究"），点输入框右侧 ⚙ 打开"高级选项"，勾掉"plan/execute 模式"和"待办列表"，再问一个检索型问题，如 `BM25 和向量检索怎么融合？`
...
- "Web 端是正式的 WebAPI + Vue 前后端分离：SSE 流式接口、PostgreSQL 存语料和历史会话——侧栏历史会话列表可以看到之前每次研究，点一下进入独立报告页回放。"

## 第五幕：后台管理（2 分钟，可选）

**操作**：侧栏底部齿轮按钮进入 `/admin`：

1. **系统状态**：仪表盘卡片（DB 连接 / 语料数 / 会话数 / 索引块数 / 运行时长），10 秒自动轮询；
2. **语料管理**：搜索框搜关键词 → 编辑某篇文档保存 → 提示"索引已自动重建：N 块"，回到前台侧栏语料块数同步变化；
3. **运维操作**：先点"立即重建"（毫秒级完成），再展示 Append/Replace 两种种子重导模式；
4. **日志查看**：级别过滤 + 自动刷新，能看到刚才每个操作的记录（研究开始/结束、重建耗时、导入明细）。

**台词**：

- "数据库里有什么，后台全能看到——语料可以直接在页面上增删改，写操作自动重建 BM25 索引，不用碰数据库也不用重启。"
- "危险操作有闸门：如果研究正在跑，Replace 清空重导会被后端 409 拒绝，前端按钮同步禁用——闸门在后端，不是只靠 UI。"
- "日志是内存环形缓冲，零依赖自研的 ILoggerProvider，重启即清——要持久化再换 Serilog，接口位置已经留好。"
